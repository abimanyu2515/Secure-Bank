...
export default function TransferMoneyPage({ onNavigate, onLogout }) {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.payeeName.trim()) newErrors.payeeName = "Beneficiary name is required.";
    if (!/^\d{9,18}$/.test(formData.accountNumber)) newErrors.accountNumber = "Enter a valid account number (9-18 digits).";
